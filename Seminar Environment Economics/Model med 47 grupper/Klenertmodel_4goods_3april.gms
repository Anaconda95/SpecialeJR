...
Z_total        sum of pollution over g 
X_total(i)     sum of consumption times price for consumer i
X_dif(i,g)     Difference between x and x0 for consumer i
...
Parameter epsilon(g) /1 0.995, 2 0.92, 3 0.98, 4 0.98/;

Parameter alpha(g)   /1 0.4, 2 0.2, 3 0.3, 4 0.25/;  

Parameter X0(g)      /1 0.0, 2 0.1,  3 0.0, 4 0/;
...
X_total.lo(i) = 1;  Z_total.lo = 1;   X_dif.lo(i,g) = min;

*Starting values
parameter initp(i) /1 0.2, 2 0.23, 3 0.24, 4 0.34, 5 0.36/;
!!p.L(i) = initp(i);

X.l(i,g)= initp(i);     !!0.3 seems like a very good starting value for 2 goods
...
GenEq(g),Utility,Welfare,Tax,Z_totaldef, X_totaldef,X_difdef, Inc_constraint,Dummy,Utility_NE,Welfare_NE
;
**********************************Firms*****************************************
FirmProd(g)..
...
HHFOC1(i,g)$(ord(g) lt 4)..  
                         pr(g)*(X(i,g)-X0(g))  =e= (alpha(g)/alpha(g+1))*((X(i,g+1)-X0(g+1))*pr(g+1));
HHFOC2(i,g)..
                         pr(g+3)*(X(i,g+3)-X0(g+3))  =e= (alpha(g+3)/gamma)*lei(i)*( (1-tau_w(i))*phi(i)*wage );                                           
HHFOC3(i)..
                         sum(g,X(i,g)*pr(g))   =e= (1-tau_w(i))*phi(i)*wage*(T_total(i)-lei(i)) + L;                        
...
Z_totaldef..
                         Z_total =e= sum(g,Z(g));
X_totaldef(i)..
                         X_total(i) =e= sum(g,(X(i,g)*pr(g)));
X_difdef(i,g)..
                         X_dif(i,g)   =e=    X(i,g)-X0(g);
                         
Inc_constraint(i)$(ord(i) lt 5)..
                         U(i+1)    =g=     prod(g,(X_dif(i,g))**alpha(g))*( T_total(i)-( X_total(i)) -L )/(( (1-tau_w(i))*phi(i+1)*wage ) )**gamma-xi*Z_total**theta;   !! Gregor approach
Utility(i)..
...
HHFOC2,HHFOC3,GenEq,numeraire,Tax,Z_totaldef, X_totaldef,X_difdef, Inc_constraint,Utility,Utility_NE,Welfare_NE,Welfare,Dummy/;
model SG_SP /totalT,FirmProd,FirmFOC1,FirmFOC2,Copeland, HHFOC1,
HHFOC2,HHFOC3,GenEq,numeraire,Tax,Z_totaldef, X_totaldef, X_difdef, Inc_constraint,Utility,Utility_NE,Welfare_NE,Welfare/;
option nlp=conopt;
...
if (SWITCH_DetL0 eq 1,
         ...
         Execute_loadpoint 'sg-initial_3goods.gdx';
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-initial_4goods.gdx';
)
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-results_4_diff_and_L_opt_4goods.gdx';
);
