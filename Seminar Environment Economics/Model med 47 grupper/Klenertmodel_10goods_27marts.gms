...
Parameter X0(g)      /1 0.0, 2 0.1,  3 0.0, 4 0, 5 0, 6 0, 7 0, 8 0, 9 0.1, 10 0/;
...
********************************************************************************
* Limits
L.lo     =       min;    T.lo(g)   =       min;    Z.lo(g)   =       min;   Z_total.lo = min;  
...
GenEq(g),Utility,Welfare,Tax,Z_totaldef,Inc_constraint,Dummy,Utility_NE,Welfare_NE
;
**********************************Firms*****************************************
FirmProd(g)..
...
Inc_constraint(i)$(ord(i) lt 5)..
                         U(i+1)    =g=     prod(g,(X(i,g)-X0(g))**alpha(g))*( T_total(i)-(sum(g,X(i,g)*pr(g)) -L )/( (1-tau_w(i))*phi(i+1)*wage ) )**gamma-xi*Z_total**theta;   !! Gregor approach
Utility(i)..
