...
SWITCH_noredist  =       1; !! no redistribution, but a tax reform
...
GenEq(g),Utility,Welfare,Tax,AltTax,Z_totaldef, X_totalconsdef, Inc_constraint,Dummy,Utility_NE,Welfare_NE
;
**********************************Firms*****************************************
FirmProd(g)..
...
********************************Government**************************************
Tax..
                         Gov_spdg   =e=    tau_p*sum(g,Z(g)) + wage*sum(i,(tau_w_preex(i)+tau_w(i)+tau_w_flat)*phi(i)*(T_total(i)-lei(i))) - 5*L; !!
AltTax..
                         Gov_spdg   =e=    wage*sum(i,(tau_w_preex(i)+tau_w(i)+tau_w_flat)*phi(i)*(T_total(i)-lei(i))) - 5*L; !!
Z_totaldef..
                         Z_total    =e=    sum(g,Z(g));
X_totalconsdef(i)..
...
model SG_ALTTAX /totalT,FirmProd,FirmFOC1,FirmFOC2,Copeland, HHFOC1,
HHFOC2,HHFOC3,GenEq,numeraire,AltTax,Z_totaldef, X_totalconsdef,Inc_constraint,Utility,Utility_NE,Welfare_NE,Welfare/;
...
if (SWITCH_DetL0 eq 1,
        ...
        !! Execute_loadpoint 'sg-results_4_diff_and_L_opt_41goods.gdx';
         Execute_loadpoint 'sg-initial_precali_41g_test.gdx';
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-initial_precali_41g_test4.gdx';
         execute '=gdx2xls sg-initial_precali_41g_test4.gdx';
...
if (SWITCH_Bench eq 1,
         ...
         Execute_loadpoint 'sg-initial_precali_41g.gdx';
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-precal_diff_and_L_opt.gdx';
);
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-precal_diff_opt.gdx';
);
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-precal_unif_opt.gdx';
);
...
!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-precali_ULS_opt.gdx';
);

if (SWITCH_noredist eq 1,
          Execute_loadpoint 'sg-initial_precali_41g_test4.gdx';
         ...
         Solve SG_AltTax      maximizing w_NE using nlp;
         Solve SG_AltTax      maximizing w_NE using nlp;
         Solve SG_AltTax      maximizing w_NE using nlp;

!!$include "SG_calculate_ginis_etc.inc"
         execute_unload 'sg-precal_noredist_test4_025.gdx';
         execute '=gdx2xls sg-precal_noredist_test4_025.gdx';
         execute '=shellExecute test.xlsx';
);
