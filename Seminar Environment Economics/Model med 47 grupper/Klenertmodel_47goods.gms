...
Parameter epsilon(g) /1 0.995, 2 0.92/;

Parameter alpha(g)   /1 0.7, 2 0.2/;  

Parameter X0(g)      /1 0, 2 0.5/;
...
F.lo(g)   =       min;   pr.lo(g)  =       min;    wage.lo  =       min;
lei.lo(i)=       min;   lei.up(i)=       24-1e-9;
X.lo(i,g+1)  =    X0(g) + min;     tau_w.up(i)=     0;

*Starting values
X.l(i,g)=0.4;
*Kan vi evt indsætte C på g's plads og så D i den ovenover? ellers giver vi dem begge startværdien 0.4, hvilket måske er for højt?
* Det kunen jeg ikke, så har i stedet sat X0 for C til 0.4. 
*tau_w.l(i)=-0.4;
...
HHFOC2(i,g)..
                         pr(g+1)*(X(i,g+1)-X0(g+1))  =e= (alpha(g+1)/gamma)*lei(i)*( (1-tau_w(i))*phi(i)*wage );                                           
HHFOC3(i)..
                         sum(g,X(i,g)*pr(g))   =e= (1-tau_w(i))*phi(i)*wage*(T_total(i)-lei(i)) + L;                        
