...
*e4(HH)..      X(HH,'D')/X(HH,'C') =e= [mu('D')+(alpha('D',hh)/p('D'))*((w*rho(hh)*N-sub))] / [mu('C')+(alpha('C',hh)/p('C'))*((w*rho(hh)*N-sub))];

e4(hh)..         p('C')*(X(HH,'c')-mu('c')) =e= alpha('c',hh)/alpha('d',hh)*(X(HH,'d')-mu('d'))*p('d');
...
alpha.fx(F,HH) = (X.l(hh,f)-mu.l(f))/(w.l*rho(hh)*N.l-sub.l)*p.l(f);
mu.fx(F) = mu.l(F);

*tjek, at det kalibreres rigtigt. Det gør det.
Parameter x_kali(hh,f); 
x_kali(hh,f)=  mu.l(f) + (alpha.l(f,hh)/p.l(f))*((w.l*rho(hh)*N.l-sub.l));
...
execute_unload 'stone_geary_5h_kal';
...
execute_unload 'stone_geary_5h_basis';

set ud  /alpha,"X", "Xrel", "U", "Urel", "EV", EVinc/;
set exp /"base","tax"/;

parameter output(exp,ud,hh,f);
parameter output2(exp,ud,hh);
parameter sigma(f), p_0(f), EVLES(hh), m(hh);

*Calculating EV from World Bank paper overview
m(hh)=w.l*rho(hh)*N.l;
p_0(f)=p.l(f);
sigma(f)=p_0(f)/(p.l(f)+tau(f));

EVLES(hh)= m(hh)*(prod(f,sigma(f)**alpha.l(f,hh))-1)+sum(f,p_0(f)*mu.l(f)) - prod(f,sigma(f)**alpha.l(f,hh))*(sum(f,(p.l(f)+tau(f))*mu.l(f)));

output('base','alpha',hh,f)=alpha.l(f,hh);
output('base','X',hh,f)=X.l(hh,f);
output('base','Xrel',hh,f)=X.l(hh,f)-output('base','X',hh,f); 
output2('base','U',hh)=prod(f,[X.l(hh,f)-mu.l(f)]**alpha.l(f,hh));
output2('base','Urel',hh)=prod(f,[X.l(hh,f)-mu.l(f)]**alpha.l(f,hh))-output2('base','U',hh);
output2('base','EV',hh)=EVLES(hh);
...
execute_unload 'stone_geary_5h_tau01';
sigma(f)=p_0(f)/(p.l(f)+tau(f));
...
output2('tax','Urel',hh)=prod(f,[X.l(hh,f)-mu.l(f)]**alpha.l(f,hh))-output2('base','U',hh);
output2('tax','EV',hh)=EVLES(hh);
output2('tax','EVinc',hh)=EVLES(hh)/m(hh);
