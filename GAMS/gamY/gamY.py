...
Available commands:
  ...
  $BLOCK <block name> <equations> $ENDBLOCK
    ...
    The group command groups together variables so that they can be manipulated together more easily, using other gamY commands such as $LOOP or $MODEL.
...
class Precompiler:
  ...
  def error(self, msg):
    list_file_name = os.path.join("LST", self.file_name.replace(".gms", ".lst"))
...
def cmd_call():
  ...
  list_file_dir = os.path.join(precompiler.file_dir, "LST")
  if not os.path.exists(list_file_dir):
    os.makedirs(list_file_dir)
  list_file_name = precompiler.file_name.replace(".gms", ".lst")
  list_file_path = os.path.join(list_file_dir, list_file_name)
