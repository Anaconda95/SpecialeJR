...
acorr <- rep(acorr,n)
decil <- rep(j-1,n)
resul <- rbind(resul, decil, alpha_sol, bstar_sol, beta_sol, acorr) 

}

vareagg = c("kod_fisk_mej","ovr_fode","bol","ene_hje","ene_tra","tra","ovr_var","tra")
...
for (j in 1:length(dataframes) ) {
  ...
  decil <- rep(j-1,n)
  resul <- rbind(resul, decil, alpha_sol, b_sol, acorr) 
...
for (j in 1:length(dataframes) ) {
 ...
 # supernum <- 1-rowSums(phat[2:T,] * sol_b_mat/10000) 
  #acorr <- rep(acorr,n)
  decil <- rep(j-1,n)
  resul <- rbind(resul, decil, alpha_sol, bstar_sol, beta_sol) 
...
for (j in 1:length(dataframes) ) {
  ...
  decil <- rep(j-1,n)
  resul <- rbind(resul, decil, alpha_sol, b_sol) 
